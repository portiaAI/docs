...
5. Once this is done you can resume the plan run using the `run` method. We have seen this `Portia` method as a way to create and kick off a `PlanRun`. In fact `run` can take a `PlanRun` in any state as a parameter and will kick off that plan run from that current state. In this particular example, it resumes the plan run from the step where the clarifications were encountered.
